...
const LoginForm = ({ role, onLogin }: LoginFormProps) => {
  ...
  if (location.state && location.state.message) {
    ...
    // ✅ Clear state after showing (important)
...
const [selectedLang, setSelectedLang] = useState("english"); // default
...
useEffect(() => {
  const fetchData = async () => {
    try {
      // 🔹 1. Fetch all videos
      const resAll = await fetch(`${API_URL}/api/videos`);
      const allData: Video[] = await resAll.json();

      const uniqueLangs = Array.from(
        new Set(
          allData
            .map((v) => (v.language || "").toLowerCase().trim())
      ...
      // 🔹 2. Decide language
      const lang = selectedLang || uniqueLangs[0];
      if (!lang) return;

      if (!selectedLang && uniqueLangs.length > 0) {
        setSelectedLang(uniqueLangs[0]);
      }

      // 🔹 3. Fetch active videos
      const resActive = await fetch(
        `${API_URL}/api/videos/active/${lang}`
      );

      const text = await resActive.text();

      let activeData: Video[] = [];
...
<div className="flex justify-end mb-5 mt-[-59px]">
  <select
    value={selectedLang}
    onChange={(e) => setSelectedLang(e.target.value)}
    className="border border-gray-300 px-4 py-2 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
  >
    {languages.length === 0 ? (
